...
Wolffish can work 100% offline with a local Ollama model — no cloud dependency at all. Cloud providers (DeepSeek, Claude, OpenAI) and Ollama are both optional; you choose what fits your setup from the model switch beside the chat input. Memory, skills, feedback — all stored locally regardless of which LLM provider you use. There's no automatic fallback between them: the model you select is the one that runs, so to work offline, flip the model switch to Local.
