...
Wolffish can work 100% offline with a local Ollama model — no cloud dependency at all. Cloud providers (DeepSeek, Claude, OpenAI) and Ollama are both optional; you choose what fits your setup by selecting it as your Brain model in Settings. Memory, skills, feedback — all stored locally regardless of which LLM provider you use. There's no automatic fallback between them: the model you select is the one that runs, so to work offline, select Ollama as your Brain.
...
This extends to runtime behavior: no hard-coded timeouts, no artificial result caps, no arbitrary limits in capability plugins. Commands run until they finish. The LLM decides when to set a timeout and how many results to request. We trust the model to make good decisions — and when it doesn't, we improve the SKILL.md instructions rather than adding code guardrails. The only hard limits are OOM protection (memory safety), not functionality restrictions.
