...
Wolffish can work 100% offline with a local Ollama model — no cloud dependency at all. Cloud providers (DeepSeek, Claude, OpenAI) and Ollama are both optional; you choose what fits your setup. Memory, skills, feedback — all stored locally regardless of which LLM provider you use. The cascade falls back gracefully: cloud to local on failure, or local-only if that's all you've configured.
