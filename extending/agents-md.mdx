...
It sits at the **root** of your data folder, next to `workspace/`, `runtime/`, and `logs/` — *not* inside `workspace/`. That keeps it clear of workspace re-initialization and makes it the first thing anyone sees when they open `~/.wolffish/`.

<Tip>
  This file isn't created automatically — it's opt-in. Paste the text once. If you reinstall, paste it again; keeping it current with a new Wolffish version is just re-copying the latest text from this page.
</Tip>

## The full AGENTS.md

Copy everything inside the block and save it as `~/.wolffish/AGENTS.md`.
