...
You don't have to open a conversation to export it: **every row on the History page carries its own bug button** too, so you can hand over a bundle for a conversation you aren't in. That button rests in two cases — the conversation you currently have open (the composer's own button belongs to that one) and a conversation whose very first turn is still running, which has nothing saved yet to collect.
...
<Tip>
Only one export runs at a time — the collectors read the same log files, so parallel runs would only fight for IO. Triggering the export again while one is in flight simply **attaches you to the running export** instead of failing it.
</Tip>
