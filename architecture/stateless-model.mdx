...
Conversations are persisted locally at `~/.wolffish/workspace/brain/conversations/` as JSON files containing the full message history, streaming segments, tool timings, and attachments. The rolling summary lives in the same file (`{summary, summarizedThroughMessage}`) as a read-time lens — the original messages are never modified or deleted, and the History page always shows the complete transcript untouched. This is purely local — providers never see a conversation ID or resume from stored state.
