...
Thalamus falls back Anthropic to OpenAI to Ollama on failure. If turn 5 fails on Anthropic and routes to OpenAI, OpenAI needs the full history — which it has, because the app owns the message array. Thread-based APIs would make mid-conversation failover impossible.
