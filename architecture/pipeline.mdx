...
7. Conditional overlays: role (orchestrator/worker), greedy/autonomous, channel formatting (WhatsApp), and a `<local_model>` honesty overlay for locally-run models
...
Full JSON schemas ship only for the **core set** — roughly 29–47 tools depending on what's connected: tool-discovery, introspect, filesystem, shell, ask, utilities (`send_file`), web-search, secrets, system, orchestrator (role-gated), and the telegram/whatsapp/electron send tools while those channels are connected. That's ~4k tokens of schemas, bringing a fresh request to ≈9.4k tokens total (vs ~94k before the lean redesign).
