...
2. Tries the primary provider (DeepSeek, Claude, OpenAI, or Ollama based on config)
...
Before each iteration's LLM call, context compaction runs to keep the growing message array within the model's context window — first a cheap truncation pass (zero cost), then an LLM-summarization pass if still over budget.

<Card title="Context Compaction" icon="compress" href="/architecture/context-compaction">
  How stale tool results and old screenshots are automatically trimmed to keep long conversations running.
</Card>
