...
4. **Response is parsed** — Wernicke's area extracts tool calls and text from the stream, normalizing across all eleven cloud provider formats
...
Maps to the **thalamus** (sensory gateway). Manages eleven cloud LLM providers plus Ollama via pure `fetch()`.
