...
Wolffish writes WhatsApp replies in WhatsApp's own text style — `*bold*`, `_italic_`, `~strikethrough~`, backtick code, and plain `-` lists — never raw Markdown. As a safety net, everything sent through the channel also passes through a converter that translates any leftover Markdown into WhatsApp text: headings become bold lines, `[label](url)` links become "label (url)", tables become `*Label:* value` lines, and task lists become ☐/☑ items. Code block contents are preserved verbatim.
