...
Since **v1.0.260** the standard is visible from the outside too. Wolffish now **offers to build a skill** when it spots you repeating a pattern, every skill it creates is stamped as its own work — a **Wolffish badge** in Settings → Capabilities, distinct from Official and imported ones — and a freshly built or freshly edited skill wears **Untested** until one of its tools **succeeds in a real call**. The badge cannot be talked away, only earned: it clears live the moment the first genuine call passes, so a skill that claims to be ready has to prove it.
