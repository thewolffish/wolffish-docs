...
<AccordionGroup>
  <Accordion title="computer-use — Screen interaction">
    ...
    **Triggers:** screenshot, click, screen, desktop, mouse, keyboard, computer use, automate, UI
    ...
    Desktop automation for visual tasks — rebuilt in **v1.0.260** around **verification instead of hope**. Every screenshot carries a **crosshair marking where the cursor is**; a small or crowded target is **zoomed** — re-captured at native resolution — before it is ever clicked; and every click, move, drag and scroll returns a **magnified close-up proving where it landed**, so a miss is visible immediately and corrected in the close-up itself with surgical precision. While Wolffish drives, the display wears a **soft blue glow** (invisible in its own screenshots) so you always know it's at the wheel, and older screenshots are retired from the model's context as fresh ones arrive, so long sessions stay grounded on the current screen — and affordable.
    ...
    **Tools:**
    ...
    - `computer_mouse_click` — Click (left/right/double); returns a magnified proof of the exact point pressed
    ...
    Must be enabled in Settings > Computer Use — and it needs a **vision-capable Brain**: the panel says so plainly when the selected model cannot see images, instead of letting a blind model click by imagination.
  </Accordion>
</AccordionGroup>
...
<Note>
  A handful of built-ins are **load-bearing** and can't be turned off: the [workflow](/configuration/workflow-mode) engine, your secrets, the `skills` capability, [projects](/configuration/projects), [automations](/configuration/heartbeat), [procedures](/configuration/procedures), the `pdf-design`, `dataviz` and `web-design` manuals, and the shared file-delivery tools the rest leans on. In **Settings → Capabilities** these wear a **Core** badge, sort to the **bottom** of the list, and show a locked **Always on** marker where their switch would be — so a stray click can't pull the floor out from under a feature you depend on. Everything you imported yourself stays fully toggleable.
</Note>
