...
| Knowledge | `hippocampus/knowledge/*.md` | Nightly promotion + `memory_save` | Yes |
...
Message arrives
  → hippocampus.appendEpisode()      [instant, no LLM]
    → brainstem daily compaction      [LLM summarizes the day's episodes]
      → knowledge promotion           [LLM extracts lasting facts, deduplicated]
```
...
1. **The learned-preferences digest** — a bounded (~500 token) summary of recent tool outcomes from the [feedback loop](/memory/feedback-loop), so checking your preferences never costs a retrieval round-trip.
