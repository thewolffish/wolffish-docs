...
description: "كيفية تنفيذ قناة اتصال جديدة لـ وولفيش"
---

# ابنِ قناة من الصفر

تربط القناة وولفيش بمنصة اتصال خارجية. واجهة سطح المكتب، Telegram، و WhatsApp كلها قنوات - تستقبل الرسائل من مصادر مختلفة لكنها توجهها عبر نفس خط أنابيب الدماغ.
...
<Info>
  يُشارك TurnRunner عبر جميع القنوات. يقوم بتسلسل الأدوار **لكل محادثة على حدة** — أدوار المحادثة الواحدة تصطف خلف بعضها، لكنّ المحادثات المختلفة تعمل بالتوازي بغض النظر عن القناة التي بدأتها.
</Info>

## التسلسل لكل محادثة

يحتفظ TurnRunner بطابورٍ واحد لكل محادثة (الأدوار بلا معرّف محادثة تحصل على مسارٍ خاصّ مفتاحه معرّف الدور). الرسالة الثانية في المحادثة نفسها تنتظر انتهاء دور تلك المحادثة الجاري — حفظًا لترتيب النصّ ومنعًا للتعارضات في ذاكرتها وتجميع سياقها — بينما رسالةٌ لمحادثة *مختلفة* تعمل فورًا وبالتوازي.

```
Desktop conversation A: "Summarize my emails" → [running] ─┐
Telegram conversation B: "What's the weather?" → [running] ─┴─ بالتوازي
Desktop conversation A: "…and reply to Sam"    → [queued behind A's turn]
```

<Warning>
  الدور طويل التشغيل يحجب فقط الرسالة التالية لمحادثته **هو** — لا المحادثات الأخرى ولا القنوات الأخرى. وتستطيع القناة أن تستبق دور محادثتها الجاري بإجهاض المتحكّم السابق قبل إرسال دور جديد.
</Warning>

## ربط المحادثات

قناتك تقرر كيف تُربط الخيوط الخارجية بمحادثات وولفيش:
