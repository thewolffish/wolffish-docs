...
يقع في **جذر** مجلد بياناتك، بجوار `workspace/` و`runtime/` و`logs/` — *وليس* داخل `workspace/`. هذا يبقيه بعيدًا عن إعادة تهيئة مساحة العمل، ويجعله أول ما يراه أي شخص يفتح `~/.wolffish/`.

<Tip>
  لا يُنشأ هذا الملف تلقائيًا — بل بطلبك. الصق النص مرة واحدة. وإن أعدت التثبيت فالصقه ثانية؛ ومواكبته لإصدار جديد من وولف فيش ما هي إلا إعادة نسخ أحدث نص من هذه الصفحة.
</Tip>
...
انسخ كل ما داخل الكتلة واحفظه في `~/.wolffish/AGENTS.md`.
