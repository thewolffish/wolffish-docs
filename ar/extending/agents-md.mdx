...
`AGENTS.md` هو ملف Markdown واحد ومكتفٍ بذاته تضعه في جذر مجلد بياناتك — `~/.wolffish/AGENTS.md` — **ليوجّه أي مساعد ذكاء اصطناعي توجّهه إلى تثبيت وولف فيش لديك**. افتح `.wolffish` في محرّر، أو سلّم المجلد لوكيل برمجي (Claude Code أو Cursor أو مساعد متصل عبر MCP)، فيقرأ هذا الملف الواحد ليفهم فورًا ما هو وولف فيش، وأين يوجد كل شيء، وكيف يساعدك.

<Note>
  لا تخلط بينه وبين [`agents.md`](/ar/configuration/agents-md) (بحروف صغيرة، في `brain/prefrontal/agents.md`). ذلك الملف يخبر **وكيل وولف فيش الخاص بك** كيف يؤدّي عمله. أما **`AGENTS.md`** (بحروف كبيرة، في جذر المجلد) فيخبر مساعدًا **زائرًا** كيف يتنقّل في *مساحة عملك*. جمهور مختلف، وملف مختلف.
</Note>

## لماذا تضعه

مساحة عمل وولف فيش ما هي إلا ملفات — قابلة للقراءة والتحرير، وملكك. هذا هو بيت القصيد: يمكنك إحضار أي ذكاء اصطناعي ليساعدك على قراءتها أو إصلاحها أو توسيعها. لكن مساعدًا باردًا لا يعرف أن `conversations/` تقع تحت `brain/`، ولا أن القدرات مسبوقة بنقطة، ولا أن `cortex.db` قابل للحذف. يضع `AGENTS.md` كل ذلك في المقدّمة ليصبح المساعد مفيدًا من الرسالة الأولى لا العاشرة.
...
| 7 | **كيف تعمل الأمور** | خط الأنابيب، وتعاقب المزوّدين، والذاكرة ثلاثية الطبقات، والأمان، ونبضة القلب، والقنوات |
...
من أكثر ما ستطلبه من مساعد زائر: *"علّم وولف فيش مهارة جديدة."* الشرح الكامل — مع مثال طقس **عامل** لا يحتاج إلى مفتاح API — موجود في **[إنشاء القدرات](/ar/extending/creating-capabilities)**. لكن إليك الفكرة كلها في أربع خطوات، وهي تقريبًا ما يخبر به `AGENTS.md` المساعد:
...
يضمّن وولف فيش هذا الدليل ويكتبه في **جذر** مجلد بياناتك، ويحدّثه عند كل إقلاع:
...
<Tip>
  هذا الملف **يديره وولف فيش** — تُعاد كتابة النسخة المضمّنة عند كل إقلاع، فتحدّثه ترقية التطبيق دائمًا إلى الإصدار الحالي. عامله كملف للقراءة فقط: فأي تعديل محلي يُستبدل عند الإقلاع التالي. وللسلوك المخصص للوكيل الذي يجب أن *يبقى*، استخدم [`agents.md`](/ar/configuration/agents-md) بدلًا منه.
</Tip>
...
هذا هو النص الذي يضمّنه وولف فيش ويبقيه متزامنًا في `~/.wolffish/AGENTS.md`، معروض هنا للرجوع إليه — اقرأه، أو ضعه يدويًا في تثبيت أقدم.
