...
<Info>
  **`In (15m)` / `In (2h)` / `In (2d)` ليست صيغة عنوان** — لا يُحلّل brainstem الصيغة `In (...)` من `heartbeat.md` إطلاقاً، فكتابتها كعنوان `##` لا تفعل شيئاً. إنها اختصار تستخدمه عند *الطلب* من وولف فيش (*"بعد يومين، ذكّرني بتجديد نطاقي"*): تُحوِّل قدرة `automations` التأخيرَ النسبي إلى `Once (...)` مطلقة عند الإنشاء — إذ لا يمكن لعدٍّ تنازلي نسبي أن يبقى بعد إعادة التشغيل — لذا ما يُكتَب فعلياً في الملف دائماً هو `Once (...)`. لكتابة مهمة لمرة واحدة يدوياً، استخدم `Once (...)`.
</Info>
...
### وضع المحادثة لكل مهمة

لكل مهمة أن تختار طريقة تشغيلها: **فردي** (نموذج واحد، مباشرةً) أو **[سير عمل](/ar/configuration/workflow-mode)** (تستطيع التشغيلة تخطيط مراحل وقيادة وكلاء متوازين). بدّله لكل مهمة من صفحة نبض القلب، أو يدوياً عبر **سطر أول اختياري** في متن كتلة المهمة:
...
سطر `mode:` يُحذَف قبل وصول التعليمات إلى الوكيل، فلا يُقرأ أبداً كجزء من المهمة. والمهمة بلا علامة تتبع وضع المحادثة النشط لحظة انطلاقها. ويضع وولف فيش العلامة بنفسه حين تطلب منه جدولة شيء (تقبل `automation_create` / `automation_edit` معامل `mode` اختيارياً).
