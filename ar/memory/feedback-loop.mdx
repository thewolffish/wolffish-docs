...
الذاكرة تُخبر Wolffish بما حدث. حلقة التغذية الراجعة تُخبره بما *نجح*. وحدة basalganglia تُسجِّل نتيجة كل تفاعل — سواء نجحت استدعاءات الأدوات، أو وافقت أو رفضت عمليات مُعلَّمة، وما هي الأساليب التي أنتجت نتائج جيدة. بمرور الوقت، يُبني هذا تفضيلات سلوكية مُتعلَّمة بدون برمجة صريحة.
...
<AccordionGroup>
  ...
  <Accordion title="العمليات الآمنة">
    إذا وافق المستخدم باستمرار (`approves`) على دفع git إلى فروع الميزات، يتعلم Wolffish أن هذه عمليات منخفضة المخاطر. إذا رفض المستخدم باستمرار (`denies`) الدفع القسري، يتعلم تجنب اقتراحها — أو تعليمها بشكل أبرز.
  </Accordion>
...
هذه هي الآلية التي تجعل Wolffish تكيفياً بمرور الوقت. إنه لا يتذكر الحقائق فقط (هذا ما تفعله ملفات المعرفة) — بل يتذكر ما *نجح عملياً*. هذا المزيج يُنشئ وكيلاً:
...
<Tip>
  حلقة التغذية الراجعة هي الأقوى لأنماط استخدام الأدوات. إذا وجدت Wolffish يقترح أسلوباً لا تحبه باستمرار، ارفضه مرة صراحة واشرح السبب. الرفض + السياق يُسجَّل ويُشكِّل السلوك المستقبلي.
</Tip>
...
- **قراءته** لفهم لماذا يتصرف Wolffish بطريقة معينة
...
<Warning>
  حذف جميع الملفات في مجلد basalganglia يُعيد تعيين جميع التفضيلات السلوكية المُتعلَّمة. سيظل لدى Wolffish ملفات المعرفة والأحداث، لكنه يفقد إحساسه بما نجح وما لم ينجح. هذه إعادة تعيين كاملة لنظام المكافآت فقط.
</Warning>
