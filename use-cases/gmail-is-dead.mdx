...
```
Check all my email accounts for unread messages. For each
account, search for unread emails in the inbox. Read the full
content of each unread email.
...
Mark all the low-priority emails as read after summarizing
them. Leave urgent and normal ones unread so they stay
visible in my inbox.
```
...
2. For each account, it calls `google_gmail_search` with `is:unread in:inbox` — one call per account, in parallel.
...
5. It calls `google_gmail_mark_read` on the low-priority emails to clear the noise.
6. It suggests reply drafts for anything that needs a response.
...
Read the full content of each unread email.
...
Mark all low-priority emails as read. Leave urgent and normal
ones unread.
```
