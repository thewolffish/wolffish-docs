...
### Claude Opus 4.x — 5 / 25 USD per MTok (input / output)
...
OpenAI models follow the same pattern — GPT-4o at 2.50/10 USD per MTok is comparable to Sonnet pricing, while o3/o4 models at 10-15/40-60 USD per MTok are premium reasoning models. OpenAI's automatic prefix caching provides a 50% discount on cached input tokens with no configuration needed.
...
These estimates assume **Claude Sonnet 4.x** with prompt caching active:
