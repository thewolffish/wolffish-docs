...
Wolffish automatically compacts the message history between agent loop iterations to prevent unbounded context growth:

- **Screenshot eviction:** keeps the 5 most recent screenshots. Older ones have their image data removed and a text marker added. The model's analysis of those screenshots (in adjacent assistant messages) is preserved.
- **Tool result truncation:** tool results older than 6 assistant turns that exceed 2,000 characters are trimmed to a 500-character prefix. Error results are never truncated.

These thresholds are conservative — short conversations (under 10 iterations) are unaffected. The compaction targets the long tail of 30-60+ iteration computer-use sessions where accumulated context would otherwise grow without bound.
...
- **Context window is the hard ceiling.** A run that accumulates more tokens than the model's context window will fail. With compaction active this ceiling is much harder to hit, but extremely long sessions (100+ iterations) with many screenshots can approach it.
