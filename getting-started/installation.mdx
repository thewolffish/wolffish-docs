...
The fastest route on any platform is the install script — it picks the right artifact, verifies it, and puts the [`wolffish` terminal command](/getting-started/cli) on your PATH:
...
<Note>
  Installing on a **server with no display**? The same install command works on a headless box — verified end to end on Ubuntu 24.04 — and the [Server Deployment](/getting-started/server-deployment) page covers the background service, autostart, and pairing your phone as the interface.
</Note>
