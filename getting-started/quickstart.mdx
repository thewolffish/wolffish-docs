...
```
DeepSeek → MiMo → Kimi → MiniMax → Claude → OpenAI → Ollama (local)
```

If the primary provider fails, Wolffish automatically falls back to the next one (skipping any that aren't configured). If you're offline and Ollama is set up, it goes straight to local inference.
