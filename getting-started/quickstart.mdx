...
In Settings → Models, add API keys for the providers you want. Then pick your **Brain** model (provider + model) from the **model switch beside the chat input** — that's the one that runs. There's no automatic fallback to other providers: the model you pick is the model that answers. To run offline, flip the switch to Local (Ollama).

Want parallel multi-agent work? The same model card carries a **Single / Workflow** chip row above its search — pick [Workflow](/configuration/workflow-mode) and the agent can plan phases and spawn parallel agents on any model you've connected. The **thinking effort** dial lives in that panel too, so every model knob is in one place.
