...
Optionally, switch the chat into **workflow mode** (`llm.mode: "workflow"`, toggled from the composer's mode pill) — each turn can then plan phases and spawn parallel agents, with the master picking every agent's model from your connected providers. See [Workflow Mode](/configuration/workflow-mode) for how that works. Even then there's no cascade — every model that runs was explicitly chosen, by you or by the master.
