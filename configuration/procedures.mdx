...
Open **Procedures** from the app menu. Each saved procedure is a card showing its **emoji icon**, its title, when it was last edited, and a preview of its prompt — most-recently-edited on top. A procedure bound to a [project](/configuration/projects) shows the project's name on its card.
...
While it runs, a **live overlay** takes over the window with a running **timer** and an **activity log** of each step; the chat is paused until the run finishes. The completed run lands in your conversation **History** marked with a **Play** icon (origin tag `[procedure]`) and stamped with the procedure's **emoji**, so you can spot whose run it was at a glance and open it later to see exactly what happened.
