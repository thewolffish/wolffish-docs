...
| Workflow | Why it's safe to schedule | Suggested cadence |
|----------|---------------------------|-------------------|
| Security audit | Read-only scan; reports findings, applies no fixes on its own | `Weekly (Sunday 06:00)` |
| Inbox briefing | Reads and summarizes; explicitly told not to modify or send | `Weekday (07:30)` |
| Media conversion | Creates new audio files; originals untouched | `Daily (09:00)` or `Every (30m)` |
| Report generation | Generates new documents; nothing destructive | `Daily (08:00)` |
