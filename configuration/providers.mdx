...
<Tip>
  If you're already using DeepSeek, Anthropic, or any other natively supported provider, keep that direct connection. Add OpenRouter only for models you can't access directly. Wolffish's cascade handles fallback automatically — direct providers are tried first.
</Tip>
...
All providers are optional — you only need at least one configured. If you use only cloud providers, the cascade skips Ollama. If you use only Ollama, it's the sole provider.
