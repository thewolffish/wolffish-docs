...
description: "Configure LLM providers: DeepSeek, Xiaomi MiMo, Kimi, Anthropic, OpenAI, and Ollama"
---

# Providers

Wolffish communicates with LLMs via six cloud providers plus a local option, all using pure `fetch()` — no SDKs. Each provider has its own streaming format and tool-calling convention, which `wernicke.ts` normalizes into a single interface.
...
Set your primary provider in Settings or `config.json`. The cascade falls back in order: **DeepSeek → MiMo → Kimi → Claude → OpenAI → Ollama** (skipping any that aren't configured). Your primary provider is tried first, then the cascade takes over on failure.

All six cloud providers and Ollama are optional — you only need at least one configured. If you use only cloud providers, the cascade skips Ollama. If you use only Ollama, it's the sole provider.
